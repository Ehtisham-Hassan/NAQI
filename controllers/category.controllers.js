...
const addCategory = asyncHandler(async (req, res) => {
    ...
    // Insert new category
    const insertCategoryQuery = "INSERT INTO category (categoryname) VALUES ($1) RETURNING *";
    const newCategory = await client.query(insertCategoryQuery, [categoryname]);

    // Send response
    return res.status(201).json(new ApiResponse(201, newCategory.rows[0], "Category added successfully"));
});
...
const updateCategory = asyncHandler(async (req, res) => {
    ...
    const updateCategoryQuery = "UPDATE category SET categoryname = $1 WHERE categoryid = $2";
    await client.query(updateCategoryQuery, [categoryname, categoryid]);

    return res.status(200).json(new ApiResponse(200, null, "Category updated successfully"));
});
...
const getCategory = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, categories.rows, "Categories fetched successfully"));
});
